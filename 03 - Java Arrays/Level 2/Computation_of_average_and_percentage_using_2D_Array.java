import java.lang.*;
import java.util.*;
public class Computation_of_average_and_percentage_using_2D_Array {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter the number of Students");
        int n = sc.nextInt();
        double[][] MarksArray = new double[n][3];
        ...
        for (int i = 0; i < n; i++) {
            double sum=0;
            for (int j = 0; j < 3; j++) {
                sum = sum + MarksArray[i][j];
                averageArray[i] = sum / 3;
                percentageArray[i] = ((sum / 300) * 100);
            }
        }
        for (int i = 0; i < n; i++) {
